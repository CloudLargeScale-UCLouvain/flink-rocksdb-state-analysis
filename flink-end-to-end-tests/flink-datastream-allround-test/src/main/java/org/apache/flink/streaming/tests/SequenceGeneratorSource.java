...
public class SequenceGeneratorSource extends RichParallelSourceFunction<Event>
    ...
    private final int workloadLength;
    ...
    private final boolean skewedData;

    private final double skewedDataSkew;
        ...
        if (context.isRestored()) {
            ...
            for (int i = rangeStartIdx; i < rangeEndIdx; ++i) {
                ...
                if (end - start > 0) {
                    keyRanges.add(
                            new KeyRangeStates(
                                    start, end, skewedData, skewedDataSkew, skewedDataBias));
                }
            }
    ...
    private static class KeyRangeStates {
        ...
        final boolean skewedData;

        final double skewedDataSkew;

        final double skewedDataBias;
        ...
        int getRandomKey(Random random) {
            if (skewedData) {
                return (int)
                        nextSkewedBoundedDouble(
                                random, startKey, endKey, skewedDataSkew, skewedDataBias);
            } else {
        ...
        public static double nextSkewedBoundedDouble(
                Random random, double min, double max, double skew, double bias) {
            double range = max - min;
            double mid = min + range / 2.0;
            double unitGaussian = random.nextGaussian();
            double biasFactor = Math.exp(bias);
            return mid
                    + (range * (biasFactor / (biasFactor + Math.exp(-unitGaussian / skew)) - 0.5));
        }
